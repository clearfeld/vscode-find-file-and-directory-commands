...
export function rg_find_file__activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("clearfeld.findFileRGPanel", async () => {
			...
			const [cmd, defaultDir] = await DetermineCMDAndDefaultDir("cd");

			provider._view.show(true);

			PostCPResultsToView(provider._view?.webview, defaultDir);
		}),
	);
...
function PostCPResultsToView(view: vscode.Webview, dir: string): void {
	let system: string;
	if (process.platform === "win32") {
		system = "win32";
	} else {
